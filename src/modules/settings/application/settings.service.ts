...
export async function updateSettings(data: Partial<Pick<UserSettings, "name" | "currency">>): Promise<ActionResult<UserSettings, "name" | "currency">> {
  const user = await getAuthUser();
  if (!user) return { status: "error", message: "Sign in to continue.", retryable: false };
  const fieldErrors: Record<string, readonly string[]> = {};
  if (data.name !== undefined && data.name.trim().length > 0 && data.name.trim().length < 2) fieldErrors.name = ["Name must be at least 2 characters."];
  if (data.currency !== undefined && data.currency.trim().length !== 3) fieldErrors.currency = ["Currency must be a 3-letter code."];
  if (Object.keys(fieldErrors).length > 0) return { status: "validation-error", fieldErrors, message: "Please fix the errors above." };

  try {
    const changes: { name?: string | null; currency?: string } = {};
    if (data.name !== undefined) changes.name = data.name.trim() || null;
    if (data.currency !== undefined) changes.currency = data.currency.trim().toUpperCase();
    ...
    if (targetCurrency && targetCurrency !== currentCurrency) {
      ...
      if (storedAmounts > 0) {
        const [row] = await db.$queryRaw<{ result: string }[]>`SELECT convert_user_currency(${user.id}, ${currentCurrency}, ${targetCurrency}, ${rate}) AS result`;
        const outcome = row?.result ?? "";
        if (outcome === "already" || outcome.startsWith("converted:")) currencyHandledInTransaction = true;
        if (outcome.startsWith("converted:")) {
          const converted = Number(outcome.slice("converted:".length));
          if (converted > 0) conversionMessage = ` Converted ${converted} amount(s) at 1 ${currentCurrency} = ${rate.toFixed(6)} ${targetCurrency}.`;
        }
      }
    }
