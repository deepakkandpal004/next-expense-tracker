...
import { computeCategoryAverages, computeSpendingForecast } from "@/src/common/domain/forecast";
import {
  computeCashFlowProjection,
  type CashFlowCategoryAverage,
  type CashFlowEvent,
  type CashFlowProjection,
} from "@/src/common/domain/cash-flow";
import { daysInResolvedPeriod } from "@/src/common/domain/reporting-period";
import type { ResolvedPeriod, TransactionType } from "@/src/common/domain/types";
import { getForecastSummaries } from "./forecast.repository";
import { getBudgetForUser } from "@/src/modules/budgets/infrastructure/budgets.repository";
import { goalReservationMinor, recurringMonthlyExpenseMinor } from "@/src/modules/dashboard/infrastructure/safe-to-spend.repository";
import { boundaryAtStart, boundaryAtEnd, toIsoDate, nextRecurrenceOccurrence } from "@/src/common/utils/date-boundaries";
...
export async function getCashFlowProjection(
  ...
  const goalContributionMinor = goals.reduce(
    (acc, goal) => acc + goalReservationMinor(goal, now), 0,
  );
  ...
  const projection = computeCashFlowProjection({
    currency, period, today, records: recordInputs, events,
    dailyVariableSpendMinor, goalContributionMinor, categoryAverages,
    budgetMinor: budget ? budget.amountMinor : null,
  });

  if (!periodEnded) return projection;

  const endBalance = projection.daily[projection.daily.length - 1]?.balanceMinor ?? 0;
  return {
    ...projection, state: "ended", daysRemaining: 0,
    daily: projection.daily.map((point) => ({ ...point, state: "recorded" })),
    projectedMonthEndMinor: endBalance, currentBalanceMinor: endBalance,
    projectedIncomeMinor: projection.actualIncomeMinor, projectedSpendMinor: projection.actualSpendMinor, alerts: [],
  };
}
