...
import type { AiInsightKind } from "@/src/common/domain/types";
...
export function setAiCompletionProviderForTesting(provider: AiCompletionProvider): () => void {
  ...
  return () => { aiCompletionProvider = previous; };
}
...
function isRetryableProviderFailure(error: unknown): boolean {
  ...
  return error instanceof Error && [
    "AbortError", "APIConnectionError", "APIConnectionTimeoutError",
...
async function runProviderAttempt<T>(
  ...
  const timeout = new Promise<never>((_resolve, reject) => {
    timer = setTimeout(() => { controller.abort(); reject(new AiProviderUnavailableError("timeout")); }, timeoutMs);
  });
...
export async function executeAiProviderRequest<T>(
  ...
  for (let attempt = 1; attempt <= policy.maxAttempts; attempt += 1) {
    try { return await runProviderAttempt(operation, policy.timeoutMs); }
    catch (error) {
      if (!isRetryableProviderFailure(error)) throw error;
      if (attempt === policy.maxAttempts) {
        throw error instanceof AiProviderUnavailableError ? error : new AiProviderUnavailableError("unavailable", { cause: error });
      }
...
const CONFIDENCE_EXPLANATION = "Confidence is the model's estimate based on the disclosed recorded-data summary; it is not a guarantee.";
...
export function toAIInsight(raw: RawInsight, id: string): AIInsight {
  ...
  return {
    id, type: asInsightKind(raw.type),
    title: raw.title?.trim() || "AI insight",
    message: raw.message?.trim() || "Analysis complete.",
    action: raw.action?.trim() || undefined,
    ...(confidence === undefined ? {} : { confidence, confidenceExplanation: raw.confidenceExplanation?.trim() || CONFIDENCE_EXPLANATION }),
    source: "ai-generated",
  };
}
...
export async function generateExpenseInsights(payload: AiProviderPayload): Promise<AIInsight[]> {
...
Disclosed period-scoped data:\n${JSON.stringify(payload, null, 2)}\nReturn only valid JSON.`;
  const response = await requestAiCompletion({
    ...
    messages: [
      { role: "system", content: "You generate clearly labeled informational spending analysis, not financial advice. Respond with valid JSON only." },
...
export async function categorizeExpense(description: string): Promise<string> {
  const response = await requestAiCompletion({
    model: "openai/gpt-3.5-turbo",
    messages: [
      { role: "system", content: "You are an expense categorization AI. Categorize expenses into one of these categories: Food, Transportation, Entertainment, Shopping, Bills, Healthcare, Other. Respond with only the category name." },
...
export async function generateGoalPlanNarration(payload: GoalPlanProviderPayload): Promise<string | null> {
...
Goal plan summary data:\n${JSON.stringify(payload, null, 2)}\nReturn only the narration text.`;
  const text = await requestAiCompletion({
    model: "openai/gpt-3.5-turbo",
    messages: [
      { role: "system", content: "You narrate an already-computed savings goal plan in plain language. You only describe the supplied numbers; you never compute or recommend investments, and you never alter the plan." },
      { role: "user", content: prompt },
    ],
...
export async function generateSafeToSpendExplanation(payload: SafeToSpendProviderPayload): Promise<string | null> {
  if (!process.env.OPENROUTER_API_KEY && !process.env.OPENAI_API_KEY) return null;
  if (payload.isDeficit) return null;
...
Safety summary data:\n${JSON.stringify(payload, null, 2)}\nReturn only the explanation text.`;
  const text = await requestAiCompletion({
    model: "openai/gpt-3.5-turbo",
    messages: [
      { role: "system", content: "You explain a calculated financial figure in everyday language. Never compute or recommend; you only narrate what was passed in." },
      { role: "user", content: prompt },
    ],
    temperature: 0.7, maxTokens: 200,
  });
...
export async function generateCanAffordVerdict(payload: CanAffordProviderPayload): Promise<string | null> {
  if (!process.env.OPENROUTER_API_KEY && !process.env.OPENAI_API_KEY) return null;
...
Affordability summary data:\n${JSON.stringify(payload, null, 2)}\nReturn only the verdict text.`;
  const text = await requestAiCompletion({
    model: "openai/gpt-3.5-turbo",
    messages: [
      { role: "system", content: "You give a friendly, honest verdict on a purchase using only the supplied summary figures. You never compute or recommend investments, and you only narrate what was passed in." },
      { role: "user", content: prompt },
    ],
    temperature: 0.7, maxTokens: 200,
  });
