...
import { isCategoryId } from '@/src/common/domain/categories';
...
const INCOME_TERMS = new Set([
  'income', 'credit', 'cr', 'in', 'revenue', 'deposit', 'refund', 'salary',
]);
const EXPENSE_TERMS = new Set([
  'expense', 'debit', 'dr', 'out', 'withdrawal', 'payment', 'spend',
]);
...
function parseAmount(raw: string | undefined, isMinor: boolean): number | null {
  ...
  if (lastDot > -1 && lastComma > -1) {
    ...
    const isThousands = cleaned.split(',').length > 2 || digitCountAfter === 3;
...
function buildRow(cells: readonly string[], roles: readonly ColumnRole[]): ParsedRow {
  ...
  roles.forEach((role, index) => {
    ...
    switch (role) {
      ...
      case 'description': if (value && row.description === undefined) row.description = value; break;
      case 'amount': row.amountRaw = value; row.isMinor = false; break;
      case 'amountMinor': row.amountRaw = value; row.isMinor = true; break;
      case 'type': row.typeRaw = value; break;
      case 'category': row.categoryRaw = value; break;
    }
  });
  return row;
}
...
export async function importTransactionsFromCsv(
  ...
  try {
    ...
    if (lines.length < 2) {
      return { status: 'error', message: 'CSV file must contain at least one data row.', retryable: false };
    }
    ...
    for (let i = startIndex; i < lines.length; i++) {
      ...
      if (!row.description) {
        ...
        if (amount === null) {
          ...
          const category = type === 'income' ? 'Income' : isIncomeCategory ? 'Other' : categoryFromRow;
          ...
          toCreate.push({ text: description, amount: finalAmount, type, category, date, userId: user.id });
          seenInFile.add(key);
