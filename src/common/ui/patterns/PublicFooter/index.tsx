      ...
      <div className="mx-auto w-full max-w-7xl">
        <div className="flex flex-wrap justify-between gap-y-12 lg:gap-x-8">
          ...
          <div className="flex w-full flex-col items-start text-left md:w-[45%] lg:w-[35%]">
            <Link href="/" className="group inline-flex items-center gap-1 rounded-2xl focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/70">
            ...
            <div className="mt-6 flex flex-col gap-2">
              {PUBLIC_NAVIGATION.map((item) => (
                <Link
                  key={item.id}
                  href={item.href}
                   className="text-sm text-white/60 transition-colors hover:text-white focus-visible:outline-none focus-visible:text-primary"
                >
                  {item.label}
                </Link>
              ))}
        ...
        {/* Divider */}
        <div className="mb-4 mt-16 h-0.5 w-full bg-gradient-to-r from-border-subtle/0 via-border-subtle to-border-subtle/0" />
