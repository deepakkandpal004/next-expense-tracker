...
export default function SignUpPage() {
  ...
  const retry = <Button intent='secondary' label='Retry account creation' onClick={() => formRef.current?.requestSubmit()} type='button' />;
  const change = (key: string, setValue: (value: string) => void) => (event: React.ChangeEvent<HTMLInputElement>) => { setValue(event.target.value); setErrors((current) => ({ ...current, [key]: undefined })); setFormError(null); };
  return <AuthenticationForm description='Create an account to start recording your financial information.' error={formError} errorAction={retry} footer={<AuthTaskLinks />} formRef={formRef} onSubmit={submit} pending={pending} pendingLabel='Creating account' submitLabel='Create account' title='Create account'><Field autoComplete='name' disabled={pending} error={errors.name} id='name' label='Full name' onChange={change('name', setName)} required value={name} /><Field autoComplete='email' disabled={pending} error={errors.email} id='email' label='Email address' onChange={change('email', setEmail)} required type='email' value={email} /><PasswordField autoComplete='new-password' description='Required. The server requires a password before it can create an account.' disabled={pending} error={errors.password} id='password' label='Password' onChange={change('password', setPassword)} value={password} /><PasswordField autoComplete='new-password' disabled={pending} error={errors.confirmation} id='confirmation' label='Confirm password' onChange={change('confirmation', setConfirmation)} value={confirmation} /></AuthenticationForm>;
}
